...
{
    public class RGG_DEB_Controller : IRGG 
    {
        private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);

        public event EventHandler<ConnectedEventArgs> SerialConnected;
        
        private static RGG_DEB_Controller _instance;

        public string Tag => "DLT";

        private const string rggClass = "RGG_DEB";
        ...
        public static RGG_DEB_Controller instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new RGG_DEB_Controller();
                }
                //log.Info("Instance");
                return _instance;
            }
        ...
        public void Initialize()
        {
            ...
            userData = new RGGUserData
            {
                ...
                GatePulseWidth = "0",
                GatePulseSO = "0",
                //AvoidSetPositionStartOffset = RggAvoidSetPositionStartOffset,
                //AvoidWidth = RggAvoidWidth,
                RcvdLUTSize = "0",
                UtcNow = "0",
                TofNow = "0",
            };
        ...
        public async Task Open()
        {
            try
            {
                await Task.Run(() => rggSerial.OpenSerial(Serial_PORT.RGG_DEB));
                IsInitialized = true;
            }
            catch (Exception ex)
            {
        ...
        private void ChangeConnected(bool bCon)
        {
            if(bCon)
            {
                ...
                {
                    log.Info(IPC_MSG.CONNECTION_RGG_DEB);
                    SerialConnected?.Invoke(this, new ConnectedEventArgs(IPC_MSG.CONNECTION_RGG_DEB));
            ...
            else
            {
                ...
                if (connected_prev != connected)
                {
                    log.Info(IPC_MSG.DISCONNECTION_RGG_DEB);
                    SerialConnected?.Invoke(this, new ConnectedEventArgs(IPC_MSG.DISCONNECTION_RGG_DEB));
                    connected_prev = connected;
                }
            }
        }
        ...
        public void OnPacketReceivedEvent(byte[] recvData)
        {
            ...
            //log.Debug($"[RGG_DEB] {BitConverter.ToString(recvData)}");

            var strPacket = (BitConverter.ToString(recvData)).Replace("-", string.Empty);
            var strDataLen = strPacket.Substring(2, 2);  //DataLength  1Byte
            ...
            foreach (string searchString in searchStrings)
            {
                ...
                {                    
                    ...
                    if (DutyModeNow.CurrentSystemObject == DutyModeNow.SystemObject.DLT)
                    {
                        if ((seq_num_rx % 10) == 0)
            ...
            if (strMSGID.Contains(CTRL))
            {
                ...
                string strRggTimeBias = ConvertEndian(strData.Substring(12, 16));
        ...
        {
            pRGG_Control = mode;
            log.Debug($"DEB_RGG_Mode를 변경합니다. [{pRGG_Control}]");
            Set_Command();
        }
        /// <summary>
        ...
        public void Cmd_GateControl(string mode, string gpw, string gpso)
        {
            ...
            log.Debug($"DEB RGG_GateControl  Width:{gpw}, StartOffset:{gpso}");
            Set_Command();
        }
        ...
        public void Set_Command()
        {
            if (connected)
            {
                ...
                Console.WriteLine($"[RGG] SetCommand : {pLEN}-{pMSG_ID}-{ConvertEndian(pRGG_Control)}-{ConvertEndian(pGatePulseWidth)}-{ConvertEndian(pGatePulseStartOffset)}" +
                    $"-{ ConvertEndian(pAvoidSetPositionStartOffset)}-{ ConvertEndian(pAvoidWidth)}-{ ConvertEndian(pLUT_UTC)}-{ConvertEndian(pLUT_Delay)}-{pCHECKSUM}");
            }
            else
            {
                //MessageBox.Show("[RGG_DEB] Set_Command() RGG Disconnction 입니다. Serial Port를 확인이 필요합니다.");
            }
        }
    ...
    observationData.StartTime, observationData.EndTime.AddSeconds(observationData.Interval_second), (int)(0.02 * 1000));
        ...
        Thread RggDEB_Tof_Thread;  // RGG LookUp Table 전송용 Thread
        ...
        public async Task Start2SendLUT()
        {
            ...
            await Task.Run(() =>
            {
                ...
                Console.WriteLine($"[RGG_DEB_Controller] LUT for [{observationData.SatelliteName} {observationData.StartTime} ~ {observationData.EndTime}] Transmission completed");
